 ...
 *
 */
import {EditorState, LexicalEditor, NodeSelection} from 'lexical';
import {
  LexicalHTMLElement,
  LexicalKey,
  RangeSelectionJSON,
...
const serializeEditorState = (editorState: EditorState) => {
  ...
  const selection: null | RangeSelectionJSON | NodeSelection = null;
...
const postEditorState = (editorState: EditorState, editor: LexicalEditor) => {
  const serializedEditorState = serializeEditorState(editorState);
  const data = {
    editor: {
      _compositionKey: editor._compositionKey,
      _config: JSON.parse(JSON.stringify(editor._config)),
      _editable: editor._editable,
      _editorState: serializedEditorState,
    },
    editorState: editor.getEditorState().toJSON(),
  };
  // eslint-disable-next-line no-console
  console.log('#1 Inject Script: postEditorState EDITOR_STATE_UPDATE', data);
  document.dispatchEvent(
    new CustomEvent(LexicalDevToolsEvents.EDITOR_STATE_UPDATE, {detail: data}),
  );
};

document.addEventListener(
  LexicalDevToolsEvents.CHECK_FOR_LEXICAL,
  function (e) {
    lexical = document.querySelectorAll('div[data-lexical-editor]').length > 0;
    const data = {lexical: lexical};
    document.dispatchEvent(
      new CustomEvent(LexicalDevToolsEvents.LEXICAL_PRESENCE_UPDATE, {
        detail: data,
      }),
    );
  },
...
document.addEventListener('loadEditorState', function (e) {
  ...
  postEditorState(initialEditorState, editor);

  editor.registerUpdateListener(({editorState}) => {
    postEditorState(editorState, editor);
  });
});
...
document.addEventListener(
  LexicalDevToolsEvents.HIGHLIGHT,
  (evt: CustomEvent) => {
    highlight(evt.detail.lexicalKey);
  },
);

document.addEventListener(LexicalDevToolsEvents.DEHIGHLIGHT, function () {
  dehighlight();
});
